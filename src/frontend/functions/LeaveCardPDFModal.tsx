...
type LeaveCardPDFModalProps = {
  isOpen: boolean;
  leave: LeaveModalRecord | null;
  onClose: () => void;
};

export default function LeaveCardPDFModal({
  isOpen,
  leave,
  onClose,
}: LeaveCardPDFModalProps) {
  if (!isOpen || !leave) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 relative">
        <h2 className="text-xl font-bold text-gray-800">Leave Card PDF</h2>
        ...
        <div className="mt-5 rounded-lg border border-dashed border-gray-300 bg-gray-50 p-4">
          <p className="text-sm text-gray-500">
            PDF modal content to be added next.
          </p>
        </div>
