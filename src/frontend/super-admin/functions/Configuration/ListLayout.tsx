...
type ListLayoutProps<T extends ListLayoutItem> = {
  title: string;
  description: string;
  ...
  onClearFilters: () => void;
  onAdd: () => void;
  onDelete: (id: T["id"], name: string) => void;
  sortValue: "a-z" | "z-a";
  onSortChange: (value: "a-z" | "z-a") => void;
  getItemName: (item: T) => string;
};

export default function ListLayout<T extends ListLayoutItem>({
  title,
  description,
  searchPlaceholder,
  addLabel,
  ...
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-2xl border border-blue-200 bg-white shadow-sm">
                  ...
                  <td className="px-3 py-1">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        aria-label={`Delete ${name || addLabel.toLowerCase()}`}
                        title="Delete"
                      >
                        <Trash2 size={12} />
                        Delete
      ...
      <div className="flex-1 min-h-0 space-y-2 overflow-y-auto p-3 md:hidden">
        {paginatedItems.map((item) => {
          const name = getItemName(item);
