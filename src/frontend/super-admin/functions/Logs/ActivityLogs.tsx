...
export default function ActivityLogs() {
    ...
    try {
      ...
      setArchiveMessage(
        count > 0
          ...
          : "No logs found for selected archive range.",
      );
  ...
  const formatAction = (action: string, details: string): string => {
    ...
    if (!normalizedAction || normalizedAction === "N/A") {
      ...
      if (pwdReset) {
        return `Reset password of ${pwdReset[1]}.`;
      }
              ...
              <>
                ...
                <p className="mt-3 text-xs text-gray-500">
                  {archiveRange.usesCustomRange
                    ? "Using selected date filter range."
                    : "No date filter selected, using default monthly archive window."}
                </p>
            ...
            {archiveStep === "confirm" && (
              <>
                <p className="mt-3 text-sm text-gray-700">
                  ...
                  {archiveRange.toLabel}? This action hides them from active
                  logs.
                </p>
