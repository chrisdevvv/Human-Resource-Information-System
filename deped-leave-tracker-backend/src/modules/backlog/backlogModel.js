...
const Backlog = {
    ...
    getById: async (id) => {
        const [rows] = await pool.promise().query('SELECT * FROM backlogs WHERE id = ?', [id]);
        return rows[0];
    },
    ...
    getByUser: async (user_id) => {
        const [rows] = await pool.promise().query(
            'SELECT * FROM backlogs WHERE user_id = ? ORDER BY created_at DESC', [user_id]
        );
        return rows;
    },
    ...
    getBySchool: async (school_id) => {
        const [rows] = await pool.promise().query(
            'SELECT * FROM backlogs WHERE school_id = ? ORDER BY created_at DESC', [school_id]
        );
        return rows;
    }
};
